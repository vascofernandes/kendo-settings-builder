...
module ksb {
    ...
    class ModelDescriptorBuilder {
        ...
        FromObject(model) {
            Object.keys(model).forEach(field => this.SetField(field, model[field]));
        }

        FromArray(model: ModelColumnDescriptor[]) {
            model.forEach(descriptor => this.SetField(descriptor.Field, descriptor.Type));
        }
    ...
    class Transport implements kendo.data.DataSourceTransport {
        read = new CrudOperation;
    }

    class DataSource implements kendo.data.DataSourceOptions {
        type = 'aspnetmvc-ajax';
    ...
    class GridBoundColumn<TModel> extends GridColumnBase {
        ...
        constructor(private type, grid: KGrid<TModel>, expression: Func<TModel, string>) {
    ...
    class GridColumnFactory<TModel> {
        ...
        constructor(private type, private container: KGrid<TModel>) {
    ...
    class GridBuilder<TModel> {
        ...
        Columns(type, configurator: Action<GridColumnFactory<TModel>>) {
